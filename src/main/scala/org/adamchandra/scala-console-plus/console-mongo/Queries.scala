 ...
 */
trait Queries extends Imports with Memoizer {
...
//  }
  ...
  implicit class UUIDEnrichments(_uuid: UUID) {
    def find(): Iterable[DBObject] = findById(_uuid)
    def findOne(coll: Option[MongoCollection] = None): DBObject = findById(_uuid, coll).headOption.getOrElse(sys.error("findOne() did not return a result"))
